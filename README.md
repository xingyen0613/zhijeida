...
`cpoep`（cpo跟）這種英文與注音相連、中間又沒有非字母鍵當線索的情形，
判別器沒有依據能決定切點，預設整串當英文。

作法不是猜，而是把可能性交給使用者：英文段落的候選表除了整串對應的中文，
也列出**尾段可能構成的字**。選了「跟」之後，該段會被拆成 `cpo` + 中文音節。
